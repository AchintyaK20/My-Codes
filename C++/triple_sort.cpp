...
void solve() {
    ...
    for (int i = 1; i <= n; i++) 
    { /// now we single out the cycles
        vector <p> v;
        while (a[i] != i) 
        { /// a[i] is the position where the element currently on position i should go
        ...
        {
            if (v.size() % 2 == 0) 
            { /// if this cycle is of even size put it into cycles
                cycles.push_back(v);
            }
            else 
            { /// if it is of odd size put it here
                oddcycles.push_back(v);
            }
        }
    }
    for (int i = 0; i < oddcycles.size(); i++) 
    { /// then add all the odd length cycles to the back so that all the cycles are sorted according to their parity
        cycles.push_back(oddcycles[i]);
    }
    if (parity % 2 == 1) 
    { /// if the permutation is odd it is impossible to sort it using given operations
        cout << "-1\n";
        return;
    }
    if (n == 3) 
    { /// base cases, probably not needed
        if (b[1] == 1) 
        {
            cout << "0\n";
        }
    ...
    for (auto f : cycles) 
    { /// from now on we just need the whole list of swaps, that is the vector 'swaps'
        for (auto g : f) 
        {
            if (g.first > g.second) 
            {
    ...
    for (int x = 0; x < swaps.size(); x += 2) 
    { /// we process the swaps two by two
